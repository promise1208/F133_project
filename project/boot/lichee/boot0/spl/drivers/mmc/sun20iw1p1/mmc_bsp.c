...
void set_mmc_para(int smc_no, void *addr, phys_addr_t uboot_base)
{
	struct spare_boot_head_t *uboot_buf =
		(struct spare_boot_head_t *)sunxi_get_iobase(uboot_base);
	...
	printf("tunning data addr:0x%x\n",
		(uint)(ulong)(uboot_buf->boot_data.sdcard_spare_data));
#else
	u32 *p = NULL;
	int i  = 0;
...
int get_card_type(void)
{
	struct boot_sdmmc_private_info_t *priv_info =
		(struct boot_sdmmc_private_info_t
			 *)sunxi_get_iobase(mmc_config_addr + SDMMC_PRIV_INFO_ADDR_OFFSET);

	return priv_info->card_type;
}

static int mmc_resource_init(u32 sdc_no)
{
...
static int _get_pll_periph0(void)
{
	...
	reg_val   = readl(IOMEM_ADDR(CCMU_PLL_PERI0_CTRL_REG));
	...
	if (data->flags & MMC_DATA_READ) {
		...
		for (i = 0; i < (byte_cnt >> 2); i++) {
			...
			if (readl(&mmchost->reg->status) & (1 << 2))
				goto out;
			buff[i] = readl(sunxi_get_iobase(mmchost->database));
		...
		for (i = 0; i < (byte_cnt >> 2); i++) {
			...
			if (readl(&mmchost->reg->status) & (1 << 3))
				goto out;
			writel(buff[i], sunxi_get_iobase(mmchost->database));
			timeout = timer_get_us() +  0xffffff;
		}
	}
...
void mmc_update_host_caps_r(int sdc_no)
{
	...
	if (priv_info->ext_para1 & EXT_PARA1_1V8_GPIO_BIAS) {
	    mmcdbg("set 1v8\n");
		writel(readl(IOMEM_ADDR(SUNXI_PIO_BASE + GPIO_POW_MODE_REG)) | (1 << 2),
		       IOMEM_ADDR(SUNXI_PIO_BASE + GPIO_POW_MODE_REG));
	}
...
//	mmcdbg("%s read(SUNXI_PIO_BASE+GPIO_POW_MODE_REG:):%x\n", __func__, readl(SUNXI_PIO_BASE + GPIO_POW_MODE_REG));
	mmcinfo("mmc %d bias %x\n", sdc_no,
		readl(IOMEM_ADDR(SUNXI_PIO_BASE + GPIO_POW_MODE_REG)));
#endif
}
